...
LIMITATIONS
-----------
- Only single-letter grid cells are supported (matches the rest of the
  app, which also assumes one letter per cell) - true rebus puzzles will
  show only the first letter of a rebus answer.
    ...
    for y in range(height):
        ...
        for x in range(width):
            ...
            if is_block(c):
                ...
                answer = c.get("answer", "")
                letter = answer[0].upper() if answer else ""
